...
class Aggregator(tf.keras.layers.Layer):
    ...
    def call(self, inputs, **kwargs):
        ...
        shape = (batch if batch is not None else -1, -1, neighbor_size, dim)
        neighbor_vectors = tf.reshape(neighbor_vectors, shape)
        neighbor_relations = tf.reshape(neighbor_relations, shape)
    ...
    def _mix_neighbor_vectors(self, neighbor_vectors, neighbor_relations, user_embeddings):
        ...
        avg = False
        if not avg:
            user_embeddings = tf.reshape(user_embeddings, shape=(-1, 1, 1, dim))  # [batch_size, 1, 1, dim]

            user_relation_scores = tf.reduce_mean(user_embeddings * neighbor_relations, axis=-1)  # [batch_size, -1, n_neighbor]
            user_relation_scores_normalized = tf.nn.softmax(user_relation_scores, axis=-1)  # [batch_size, -1, n_neighbor]
            ...
            neighbors_aggregated = tf.reduce_mean(neighbor_vectors, axis=2)  # [batch_size, -1, dim]
...
class SumAggregator(Aggregator):
    ...
    def _call(self, self_vectors, neighbor_vectors, neighbor_relations, user_embeddings, **kwargs):
        batch, dim = user_embeddings.shape
        neighbors_agg = self._mix_neighbor_vectors(neighbor_vectors, neighbor_relations, user_embeddings)  # [batch_size, -1, dim]

        output = tf.reshape(self_vectors + neighbors_agg, shape=(-1, dim))  # [-1, dim]
        #if kwargs['training']:
        #    output = tf.nn.dropout(output, rate=0.2)
        output = tf.nn.bias_add(tf.matmul(output, self.kernel), self.bias)  # [-1, dim]

        return tf.reshape(output, shape=(batch if batch is not None else -1, -1, dim))  # [batch_size, -1, dim]
