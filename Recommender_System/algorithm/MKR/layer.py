...
class CrossCompressUnit(tf.keras.layers.Layer):
    def __init__(self, weight_regularizer, **kwargs):
        super(CrossCompressUnit, self).__init__(**kwargs)
        self.weight_regularizer = tf.keras.regularizers.get(weight_regularizer)

    def build(self, input_shape):
        self.dim = input_shape[0][-1]
        self.weight_vv = self.add_weight(shape=(self.dim, 1), regularizer=self.weight_regularizer, name='weight_vv')
        self.weight_ev = self.add_weight(shape=(self.dim, 1), regularizer=self.weight_regularizer, name='weight_ev')
        self.weight_ve = self.add_weight(shape=(self.dim, 1), regularizer=self.weight_regularizer, name='weight_ve')
        self.weight_ee = self.add_weight(shape=(self.dim, 1), regularizer=self.weight_regularizer, name='weight_ee')
        self.bias_v = self.add_weight(shape=self.dim, initializer='zeros', name='bias_v')
        self.bias_e = self.add_weight(shape=self.dim, initializer='zeros', name='bias_e')
    ...
    def call(self, inputs):
        ...
        v_out = tf.reshape(tf.matmul(c_matrix, self.weight_vv) + tf.matmul(c_matrix_t, self.weight_ev),
                           shape=[-1, self.dim]) + self.bias_v  # (batch, dim)
        e_out = tf.reshape(tf.matmul(c_matrix, self.weight_ve) + tf.matmul(c_matrix_t, self.weight_ee),
                           shape=[-1, self.dim]) + self.bias_e  # (batch, dim)
