...
def DeepFM_model(n_user: int, n_item: int, dim=8, layers=[16, 16, 16], l2=1e-6) -> tf.keras.Model:
    print('初始化DeepFM模型：n_user=', n_user, ', n_item=', n_item, ', dim=', dim, ', layers=', layers, ', l2=', l2, sep='')
    ...
    for layer in layers:
        deep = tf.keras.layers.Dense(layer, activation='relu', kernel_regularizer=reg_l2(l2))(deep)
    deep = tf.keras.layers.Dense(1, kernel_regularizer=reg_l2(l2))(deep)
